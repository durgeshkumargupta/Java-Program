...
public class StudentDao {
	...
	public static int save(Student s){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into employee(name,email,course,fee,paid,due,address,city,state,country,contactno) values(?,?,?,?,?,?,?,?,?,?,?)");
	...
	public static int update(Student s){
		...
		try{
			Connection con=AccountantDao.getCon();
			PreparedStatement ps=con.prepareStatement("update employee set name=?,email=?,course=?,fee=?,paid=?,due=?,address=?,city=?,state=?,country=?,contactno=? where rollno=?");
			ps.setString(1,s.getName());
			ps.setString(2,s.getEmail());
			ps.setString(3, s.getCourse());
			ps.setInt(4,s.getFee());
	...
	public static Student getStudentByRollno(int rollno){
		...
		try{
			Connection con=AccountantDao.getCon();
			PreparedStatement ps=con.prepareStatement("select * from employee where rollno=?");
			ps.setInt(1,rollno);
			ResultSet rs=ps.executeQuery();
			if(rs.next()){
				s.setRollno(rs.getInt("rollno"));
	...
	public static List<Student> due(){
		...
		try{
			Connection con=AccountantDao.getCon();
			PreparedStatement ps=con.prepareStatement("select * from employee where due>0");
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
				Student s=new Student();
				s.setRollno(rs.getInt("rollno"));
